...
export default function Blogs() {
    ...
    fetch(`${baseUrl}/blogs?populate=Image`)
        .then(res => res.json())
        .then(data => {
        console.log('Fetched Blogs:', data);
        const parsed = data.data.map((item) => item);
        setBlogs(parsed.filter(project => !project.Dabbled));
        })
        .catch(err => {
                            ...
                            {blogs.map(blog  => {
                                ...
                                return (
                                    ...
                                    <div>
                                        ...
                                        {attrs.Date 
                                            ? new Date(attrs.Date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) 
                                            : ''}
                                        </p>
